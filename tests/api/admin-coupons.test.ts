...
describe("admin coupons", () => {
  it("403s for a non-admin", async () => {
    const res = await createCoupon(jsonRequest("http://localhost/api/admin/coupons", "POST", payload(), retailCookie), ctx());
    expect(res.status).toBe(403);
  });

  it("creates a coupon", async () => {
    const res = await createCoupon(jsonRequest("http://localhost/api/admin/coupons", "POST", payload(), adminCookie), ctx());
  ...
  it("409s on a duplicate code", async () => {
    await createCoupon(jsonRequest("http://localhost/api/admin/coupons", "POST", payload(), adminCookie), ctx());
    const res = await createCoupon(jsonRequest("http://localhost/api/admin/coupons", "POST", payload(), adminCookie), ctx());
  ...
  it("409s when patching a coupon to a colliding code", async () => {
    const other = await (
      await createCoupon(jsonRequest("http://localhost/api/admin/coupons", "POST", payload({ code: "OTHER10" }), adminCookie), ctx())
    ).json();
