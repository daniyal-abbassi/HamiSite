...
describe("PATCH /api/auth/me", () => {
  ...
  it("leaves omitted fields untouched and clears a field sent as null", async () => {
    await updateMe(jsonRequest(URL, "PATCH", { firstName: "Ali", city: "Tehran" }, cookie), ctx());
    await updateMe(jsonRequest(URL, "PATCH", { city: null }, cookie), ctx());
  ...
  it("returns 409 DUPLICATE_ACCOUNT for another user's email", async () => {
    ...
    const res = await updateMe(jsonRequest(URL, "PATCH", { email: "boss@example.com" }, cookie), ctx());
    expect(res.status).toBe(409);
    expect((await res.json()).error.code).toBe("DUPLICATE_ACCOUNT");
  });

  it("allows re-submitting your OWN current email", async () => {
    await updateMe(jsonRequest(URL, "PATCH", { email: "mine@example.com" }, cookie), ctx());
    const res = await updateMe(jsonRequest(URL, "PATCH", { email: "mine@example.com", city: "Shiraz" }, cookie), ctx());
    expect(res.status).toBe(200);
  });

  it("returns 400 for an empty body, an invalid email, and malformed JSON", async () => {
    expect((await updateMe(jsonRequest(URL, "PATCH", {}, cookie), ctx())).status).toBe(400);
    expect((await updateMe(jsonRequest(URL, "PATCH", { email: "not-an-email" }, cookie), ctx())).status).toBe(400);
  ...
  it("returns 401 AUTH_REQUIRED without a session", async () => {
    const res = await updateMe(jsonRequest(URL, "PATCH", { firstName: "Ali" }), ctx());
  ...
  it("refreshes the session cookie on the response (sliding expiry via withAuth)", async () => {
    const res = await updateMe(jsonRequest(URL, "PATCH", { firstName: "Ali" }, cookie), ctx());
    expect(res.headers.get("set-cookie")).toContain("Path=/");
    expect((await me(getRequest(URL, cookie), ctx())).status).toBe(200);
  });
});
...
describe("POST /api/auth/change-password", () => {
  it("changes the password; the old one stops working and the new one works", async () => {
    const res = await changePassword(
      jsonRequest(CP_URL, "POST", { currentPassword: seed.retail.password, newPassword: "NewPassword@456" }, cookie), ctx());
    expect(res.status).toBe(200);
  ...
  it("revokes every OTHER session and keeps the current one alive", async () => {
    ...
    expect((await me(getRequest(URL, cookie), ctx())).status).toBe(200); // current: alive
    expect((await me(getRequest(URL, otherCookie), ctx())).status).toBe(401); // other: revoked
  });

  it("returns 400 INVALID_CURRENT_PASSWORD (not 401) for a wrong current password", async () => {
    const res = await changePassword(
      jsonRequest(CP_URL, "POST", { currentPassword: "not-my-password", newPassword: "NewPassword@456" }, cookie), ctx());
  ...
  it("rejects a new password identical to the current one", async () => {
    const res = await changePassword(
      jsonRequest(CP_URL, "POST", { currentPassword: seed.retail.password, newPassword: seed.retail.password }, cookie), ctx());
    expect(res.status).toBe(400);
  ...
  it("rejects a new password under 6 characters", async () => {
    const res = await changePassword(
      jsonRequest(CP_URL, "POST", { currentPassword: seed.retail.password, newPassword: "abc12" }, cookie), ctx());
    expect(res.status).toBe(400);
  ...
  it("returns 401 AUTH_REQUIRED without a session and 400 MALFORMED_JSON on bad JSON", async () => {
    expect(
      (await changePassword(jsonRequest(CP_URL, "POST", { currentPassword: "x", newPassword: "NewPassword@456" }), ctx()))
        .status,
    ).toBe(401);

    const bad = await changePassword(
      new Request(CP_URL, { method: "POST", headers: { "content-type": "application/json", cookie }, body: "nope" }), ctx());
    expect(bad.status).toBe(400);
    expect((await bad.json()).error.code).toBe("MALFORMED_JSON");
  });
});
