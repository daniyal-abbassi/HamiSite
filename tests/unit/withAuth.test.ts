...
describe("withAuth", () => {
  it("returns 401 with no cookie", async () => {
    ...
    const res = await handler(requestWithCookie(), ctx());
    expect(res.status).toBe(401);
  });

  it("returns 401 for an unknown or expired token", async () => {
    const handler = withAuth(async () => NextResponse.json({ ok: true }));
    const res = await handler(requestWithCookie("not-a-real-token"), ctx());
    expect(res.status).toBe(401);
    ...
    const expiredRes = await handler(requestWithCookie(token), ctx());
  ...
  it("returns 403 for a deactivated account", async () => {
    ...
    const { token } = await createSession(seed.retail.id);
    const handler = withAuth(async () => NextResponse.json({ ok: true }));
    const res = await handler(requestWithCookie(token), ctx());
  ...
  it("threads real route params through to the handler", async () => {
    ...
    const res = await handler(requestWithCookie(token), ctx({ id: "42" }));
    expect(res.status).toBe(200);
    const body = await res.json();
  ...
  it("tags its 401/403 failures with stable codes", async () => {
    ...
    expect((await (await handler(requestWithCookie(), ctx())).json()).error.code).toBe("AUTH_REQUIRED");

    const { token } = await createSession(seed.retail.id);
    expect((await (await handler(requestWithCookie(token), ctx())).json()).error.code).toBe("FORBIDDEN_ROLE");

    // isActive is checked before roles, so this now reports deactivation.
    await prisma.user.update({ where: { id: seed.retail.id }, data: { isActive: false } });
    expect((await (await handler(requestWithCookie(token), ctx())).json()).error.code).toBe("ACCOUNT_DEACTIVATED");
  });
});
