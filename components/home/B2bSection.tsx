          ...
          <div>
            ...
            <div className="mt-6 flex flex-wrap gap-2" aria-label="حوزه‌های تأمین">
              {b2bSupplyAreas.map((area) => (
                <span key={area} className="rounded-full border border-aqua/35 bg-aqua/5 px-3 py-1.5 text-[11px] font-bold text-aqua">
                  {area}
                </span>
              ))}
            </div>
            <div className="mt-7 flex flex-wrap gap-3">
              ...
              <Link
                href="/partners"
                className="inline-flex items-center gap-1.5 rounded-full border border-aqua/50 px-5 py-2.5 text-sm font-bold text-aqua transition-colors hover:bg-aqua/10"
              >
            ...
            {b2bFeatures.map((feature, index) => (
              <Reveal key={feature.title} delay={index * 70}>
                <article className="glass h-full rounded-2xl p-5">
                  <PackageCheck className="size-5 text-aqua" strokeWidth={1.5} aria-hidden="true" />
