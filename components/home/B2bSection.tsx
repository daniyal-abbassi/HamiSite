  ...
  return (
    ...
    >
      <div className="container">
          ...
          <div className="grid gap-10 lg:grid-cols-[1.1fr_0.9fr]">
            <div>
              ...
              <p className="mt-4 max-w-md text-sm leading-8 text-foreground/65">
                ...
                طراحی شده است؛ از ثبت‌نام تا سفارش، در سه گام روشن.
              </p>
              <div
                className="mt-6 flex flex-wrap gap-2"
                aria-label="حوزه‌های تأمین"
              >
                {b2bSupplyAreas.map((area) => (
                  <span
                    key={area}
                    className="rounded-full border border-aqua/35 bg-aqua/5 px-3 py-1.5 text-xs font-bold text-aqua"
                  >
                    {area}
                  </span>
                ))}
              ...
              <div className="mt-7 flex flex-wrap gap-3">
                {/* One primary action. The old pair pointed the B2B funnel's
                    first click at /partners/login, a route that does not exist
                    (the partner flow is the application form; there is no
                    partner panel to log into yet). Until that page ships, the
                    application form IS the entry, so the CTA points at it. */}
                <Link
                  href="/partners"
                  className={buttonVariants({ variant: "oxblood" })}
                >
                  ثبت‌نام همکاری <ArrowLeft className="size-4" />
                </Link>
              </div>
            </div>
        ...
        <Reveal delay={120}>
          <ol
            className="mt-12 grid list-none gap-6 border-t border-line p-0 pt-10 sm:grid-cols-3"
            aria-label="فرآیند همکاری"
          >
            {b2bWorkflow.map((step) => (
              <li key={step.index} className="flex items-start gap-4">
                <span className="font-mono text-2xl font-medium text-aqua/80">
                  {step.index}
                </span>
                <div>
